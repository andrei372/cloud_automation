...
def create_instance(compute, project, zone, name, bucket, puppet_master_ip, puppet_network_subnet):
    ...
    startup_script = open(
        os.path.join(
            os.path.dirname(__file__), 'puppet-master-startup-script.sh'), 'r').read()
...
# Main
def main(project, bucket, zone, instance_name, network_subnet, static_ip, wait=False):
    ...
    print('\nCreating Puppet Master instance...')

    operation = create_instance(compute, project, zone, instance_name, bucket, ops_master_ip, ops_network_subnet)
    ...
    print("""
Puppet Master Instance created.
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'network_static_ip', 
        help='Your Google Cloud Network internal static IP CID notation. Example 10.10.10.100')

    args = parser.parse_args()
    
    # run main function 
    main(args.project_id, args.bucket_name, args.zone, args.instance_name, args.network_subnet_name, args.network_static_ip)
